#!/usr/bin/env python3
"""SAMSum summarization evaluator with multiple RAG/RLM settings.

Modes:
- rag: one-pass transcript summarization with retrieved context
- ga: prompt-improved RAG with a genetic-style instruction
- es: prompt-improved RAG with an exploration-style instruction
- repl4: short REPL/RLM over retrieved transcript snippets
- repl8: longer REPL/RLM over retrieved transcript snippets
...
DEFAULT_DATA = PROJECT_DIR / "Data" / "SAMSum" / "train" / "train.csv"
DEFAULT_OUT_ROOT = PROJECT_DIR / "samsum_outputs"
DEFAULT_QUANTIZED_MODEL = "Meta-Llama-3-8B-Instruct.Q4_K_M.gguf"

MODE_META = {
    "rag": ("SAMSum Llama 3 8B + RAG", "samsum_rag"),
    "ga": ("SAMSum GA + RAG", "samsum_ga_rag"),
    "es": ("SAMSum ES + RAG", "samsum_es_rag"),
    "repl4": ("SAMSum RLM REPL 4-step + RAG", "samsum_rlm_repl4_rag"),
    "repl8": ("SAMSum RLM REPL 8-step + RAG", "samsum_rlm_repl8_rag"),
    "adaptive_p": ("SAMSum Adaptive-P short REPL + RAG", "samsum_adaptive_p_short_repl_rag"),
}
...
def load_rows(path: Path, max_rows: int | None) -> pd.DataFrame:
    print(f"[setup] Loading SAMSum: {path}", flush=True)
    df = pd.read_csv(path)
    if max_rows:
        df = df.head(max_rows)
...
def normalize_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df.reset_index(drop=True).iterrows():
        sample_id = safe_str(row.get("sample_id")) or f"SAMSum_{idx}"
        transcript = safe_str(row.get("transcript"))
        gt_summary = safe_str(row.get("gt_summary"))
        baseline_summary = safe_str(row.get("llm_summary"))
        rows.append(
            {
                "row_number": idx + 1,
                "sample_id": sample_id,
...
def build_vectorstore(df: pd.DataFrame, cfg: EvalConfig) -> tuple[Chroma, bool]:
    persist_dir = cfg.out_dir / "chroma_samsum"
    rebuilt = False
    if cfg.rebuild_index and persist_dir.exists():
        shutil.rmtree(persist_dir)
    embeddings = HashEmbeddings()
    if persist_dir.exists():
        print(f"[vector-db] Reusing SAMSum index: {persist_dir}", flush=True)
        return Chroma(persist_directory=str(persist_dir), embedding_function=embeddings), rebuilt
    print(f"[vector-db] Creating SAMSum index with {len(df)} documents", flush=True)
    docs = [
        Document(
...
def model_instruction(mode: str) -> str:
    if mode == "ga":
        return (
            "Use an evolved concise summarization policy: preserve all participants, "
            "decisions, requests, times, places, and commitments. Remove chit-chat."
        )
    if mode == "es":
        return (
            "Explore several possible summaries mentally, then return the one with "
            "the fewest missing facts and no unsupported claims."
        )
    return "Write a faithful concise SAMSum-style dialogue summary."
...
def answer_once(llm: Llama | None, row: Dict[str, Any], docs: List[Document], cfg: EvalConfig) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        "Return only valid JSON with keys summary, confidence, evidence_sample_ids.\n\n"
        f"Transcript to summarize:\n{row['transcript']}\n\n"
        f"Retrieved similar dialogue context, for style only:\n{retrieved_context}"
    )
    raw = ask_llm(
        llm,
        [
            {"role": "system", "content": "You are a faithful dialogue summarizer."},
...
def sub_rlm(
    ...
    prompt = (
        "Extract only summary-worthy facts from this dialogue snippet. Focus on "
        "participants, actions, decisions, requests, dates/times, places, and outcomes. "
    ...
    raw = ask_llm(
        llm,
        [
            {"role": "system", "content": "You are a sub-RLM evidence extractor for dialogue summarization."},
            {"role": "user", "content": prompt},
        ],
        cfg,
...
def root_aggregate(
    ...
    prompt = (
        "Create the final SAMSum summary from the transcript and extracted facts. "
        "The final summary must be concise, faithful, and cover all important facts. "
        "Do not add unsupported facts. Return only valid JSON with keys summary, "
    ...
    raw = ask_llm(
        llm,
        [
            {"role": "system", "content": "You are the root RLM summarizer."},
            {"role": "user", "content": prompt},
        ],
        cfg,
    )
...
def refine_summary(
    ...
    if cfg.dry_run or not current:
        return parsed, ""
    prompt = (
        "Refine the current summary using the metric feedback. Improve missing facts "
        "and factuality, but keep it concise. Return only valid JSON with key summary.\n\n"
        f"Transcript:\n{row['transcript']}\n\n"
    ...
    raw = ask_llm(
        llm,
        [
            {"role": "system", "content": "You are a precise summary refiner."},
            {"role": "user", "content": prompt},
        ],
        cfg,
...
def judge_answer(llm: Llama | None, row: Dict[str, Any], parsed: Dict[str, Any], cfg: EvalConfig) -> Dict[str, Any]:
    ...
    prompt = (
        "Grade this SAMSum summary against the reference summary and transcript. "
...
def run(cfg: EvalConfig) -> None:
    ...
    if not cfg.dry_run and not cfg.model_path:
        print(f"[llm] SAMSum default quantized model: {DEFAULT_QUANTIZED_MODEL}", flush=True)
    llm = load_llm(_Cfg())
