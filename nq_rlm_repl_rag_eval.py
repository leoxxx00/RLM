...
PROJECT_OUT_DIR = Path("/Users/htet/Desktop/Projects/X-RLM/nq_rlm_repl_rag_outputs")
...
def run(cfg: RlmConfig) -> None:
    ...
    for index, row in enumerate(df.to_dict("records"), start=1):
        ...
        add_paper_columns(
            out_row,
            metric_prefix="rlm",
            method_mapping="REPL-style Recursive Language Model over external P snippets.",
            principles=(
                ...
                "metadata feedback, sub_RLM recursion, compound-call budget, and "
                "fused faithfulness score."
            ),
    ...
    results_path = cfg.out_dir / "nq_rlm_repl_rag_assessment_traces.csv"
    ...
    summary.insert(1, "method", "RLM_REPL_RAG")
    summary.insert(
        2,
        "summary_explanation",
        (
            "Mean metrics for Algorithm-1 REPL-style RLM: LLM-generated safe "
            "REPL actions over external P, sub_RLM calls, stdout metadata history, "
            "and final root aggregation."
        ),
    )
    ...
    summary_path = cfg.out_dir / "nq_rlm_repl_rag_summary.csv"
