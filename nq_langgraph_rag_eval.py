...
def make_graph(vectorstore: Chroma, llm: Llama | None, cfg: EvalConfig):
    ...
    def prompt_node(state: RagState) -> RagState:
        ...
        trace = state.get("trace", []) + [
            {"node": "prompt", "chars": len(prompt), "prompt": prompt}
        ]
        return {**state, "prompt": prompt, "trace": trace}
...
def run(cfg: EvalConfig) -> None:
    ...
    for index, row in enumerate(df.to_dict("records"), start=1):
        ...
        out_row = {
            "row_number": index,
            "experiment_name": "Llama 3 8B + RAG",
            "row_explanation": (
                "One Natural Questions item evaluated with Chroma retrieval, a "
                "single Llama answer call, automatic metrics, and LLM-as-judge."
            ),
            "sample_id": first_text(row.get("sample_id", "")),
            "question": question,
            "answer_summary": (
            ...
            "trace_summary": "retrieve -> prompt -> llm -> assess -> llm_judge",
        ...
        add_paper_columns(
            out_row,
            metric_prefix="rag",
            method_mapping="Baseline RAG plus faithfulness metric fusion.",
            principles=(
                "Faithfulness metric fusion combines automatic metrics and LLM-as-judge; "
                "agentic reasoning is represented by retrieve->answer->judge workflow."
            ),
            estimated_lm_calls=1 + (0 if cfg.skip_llm_judge else 1),
        )
        rows.append(out_row)
        print(
    ...
    results_path = cfg.out_dir / "nq_rag_llama_assessment_traces.csv"
    ...
    summary.insert(1, "method", "LLAMA3_RAG")
    summary.insert(2, "summary_explanation", "Mean metrics for plain Llama 3 8B RAG.")
    ...
    print(f"[output] Writing summary CSV: {cfg.out_dir / 'nq_rag_llama_summary.csv'}", flush=True)
    summary.to_csv(cfg.out_dir / "nq_rag_llama_summary.csv", index=False)
    print(f"[done] Total elapsed: {time.time() - total_started:.2f}s", flush=True)
    print(f"[done] Wrote: {results_path}", flush=True)
    print(f"[done] Wrote: {cfg.out_dir / 'nq_rag_llama_summary.csv'}", flush=True)
    print(f"[done] Wrote: {cfg.out_dir / 'dataset_profile.csv'}", flush=True)
