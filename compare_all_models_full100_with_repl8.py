#!/usr/bin/env python3
"""Compare all available Natural Questions RAG/RLM model traces on 100 rows.

This version is pointed at the separate RLM REPL 8-step full-100 output folder,
so the 8-step experiment can be judged fairly after it finishes all 100 rows.
"""
...
DEFAULT_OUT_DIR = PROJECT_DIR / "nq_clean_monitor_exports" / "all_models_full100_with_repl8"
...
TRACE_FILES = {
    ...
    "RLM REPL 8-step full100 + RAG": {
        "path": PROJECT_DIR
        / "nq_rlm_repl8_full100_rag_outputs"
        / "nq_rlm_repl8_full100_rag_assessment_traces.csv",
        "prefix": "rlm",
    },
...
COLORS = {
    ...
    "ES + RAG": "#d62728",
    "RLM REPL + RAG": "#9467bd",
    "RLM REPL 8-step full100 + RAG": "#8c564b",
...
def write_csvs(df: pd.DataFrame, out_dir: Path, max_rows: int) -> pd.DataFrame:
    ...
    long_path = out_dir / "all_models_full100_with_repl8_trace_rows_long.csv"
    ...
    summary_path = out_dir / "all_models_full100_with_repl8_summary.csv"
    ...
    chart_path = out_dir / "all_models_full100_with_repl8_chart_ready_long.csv"
...
def plot_quality(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("All available models, 100-question comparison")
    ax.grid(axis="y", alpha=0.25)
    ax.legend(frameon=False, ncol=2)
    fig.tight_layout()
    path = out_dir / "all_models_full100_with_repl8_quality_metrics.png"
...
def plot_runtime(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Runtime, 100-question comparison")
    ax.grid(axis="y", alpha=0.25)
    plt.xticks(rotation=20, ha="right")
    fig.tight_layout()
    path = out_dir / "all_models_full100_with_repl8_runtime.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def plot_fused(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Balanced fused ranking, 100-question comparison")
    ax.grid(axis="y", alpha=0.25)
    plt.xticks(rotation=20, ha="right")
    fig.tight_layout()
    path = out_dir / "all_models_full100_with_repl8_balanced_fused_score.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def plot_short_long(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Short vs long F1, 100-question comparison")
    ax.grid(alpha=0.25)
    fig.tight_layout()
    path = out_dir / "all_models_full100_with_repl8_short_vs_long_f1.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def main() -> None:
    ...
    print("\n=== All models full-100-with-REPL8 summary ===")
    print(
        summary[
            [
                "model",
