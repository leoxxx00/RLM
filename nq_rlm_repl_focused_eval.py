...
PROJECT_OUT_DIR = Path("/Users/htet/Desktop/Projects/X-RLM/nq_rlm_repl_focused_outputs")
...
def make_environment(question: str, docs: list[Any]) -> Dict[str, Any]:
    ...
    prompt = {
        ...
        "snippets": snippets,
        "instruction": (
            "Run the focused three-job REPL: understand the question, extract "
            "answer evidence, then generate the final answer. Return JSON with "
...
def subcall_prompt(question: str, snippet: Dict[str, Any]) -> str:
    return (
        "You are the evidence-extraction job in a focused RLM REPL scaffold. "
        "Inspect only this snippet and decide whether it answers the question. "
        "Prefer exact answer spans and concise supporting evidence. Return only "
        "valid JSON with keys snippet_id, sample_id, relevant, "
        "short_answer_candidate, long_answer_evidence, evidence_quote, "
...
def final_prompt(
    ...
    return (
        "You are the final-answer generation job in a focused RLM REPL scaffold. "
        "The full prompt/evidence is stored outside your context as variable P. "
        "You have question understanding, metadata, and recursive evidence results. "
        "Generate the final answer from the strongest relevant evidence only. "
        "Return only valid JSON with keys long_answer, short_answer, answer_summary, "
        "confidence, evidence_sample_ids, and reasoning_summary.\n\n"
        f"Question: {question}\n\n"
        f"Question understanding:\n{json.dumps(question_understanding or {}, ensure_ascii=False)}\n\n"
...
def question_understanding_prompt(question: str, environment_metadata: Dict[str, Any]) -> str:
    return (
        "You are the question-understanding job in a focused RLM REPL scaffold. "
        "Rewrite the user's question into a precise retrieval and answering plan. "
        "Do not answer yet. Return only valid JSON with keys rewritten_question, "
        "question_type, expected_answer_type, key_entities, search_terms, "
        "answer_constraints, and ambiguity_notes.\n\n"
        f"Question: {question}\n\n"
        f"External P metadata:\n{json.dumps(environment_metadata, ensure_ascii=False)}"
    )
...
def understand_question(
    ...
    prompt = question_understanding_prompt(question, environment_metadata)
    if cfg.dry_run:
        tokens = re.findall(r"[a-z0-9]+", question.lower())
        parsed = {
            "rewritten_question": question,
            "question_type": "factoid",
            "expected_answer_type": "short factual answer",
            "key_entities": tokens[:8],
            "search_terms": tokens[:8],
            "answer_constraints": "Answer only what the question asks.",
            "ambiguity_notes": "",
        }
        ...
        raw = ask_llm(
            ...
            [
                {
                    "role": "system",
                    "content": "You are a precise question-understanding module.",
                },
    ...
    parsed.setdefault("rewritten_question", question)
    parsed.setdefault("search_terms", re.findall(r"[a-z0-9]+", question.lower())[:8])
    parsed.setdefault("expected_answer_type", "")
    return parsed, raw, prompt


def ranked_handles_for_understanding(
    question: str,
    state: Dict[str, Any],
    question_understanding: Dict[str, Any],
) -> List[str]:
    classification = batch_classify_snippets_action(question, state)["classifications"]
    terms = question_understanding.get("search_terms", [])
    if isinstance(terms, str):
        terms = re.findall(r"[a-z0-9]+", terms.lower())
    ...
    for item in classification:
        snippet = snippets.get(item["handle"], {})
        snippet_text = str(snippet.get("text", "")).lower()
        term_hits = sum(1 for term in term_set if term and term in snippet_text)
        score = float(item.get("overlap_score", 0)) + (2.0 * term_hits)
        ranked.append({**item, "focused_score": score, "term_hits": term_hits})
    ranked.sort(key=lambda item: item["focused_score"], reverse=True)
    state["snippet_classifications"] = ranked
    return [item["handle"] for item in ranked[: state["top_k"]] if item.get("handle")]
...
def generate_final_answer(
    ...
    if cfg.dry_run:
        relevant = [
            item for item in state.get("sub_results", [])
            if str(item.get("relevant", "")).lower() in {"true", "1", "yes"}
        ]
        best = relevant[0] if relevant else (state.get("sub_results", [{}])[0] if state.get("sub_results") else {})
        parsed = {
            "long_answer": str(best.get("long_answer_evidence", ""))[:900],
            "short_answer": str(best.get("short_answer_candidate", "")),
            "answer_summary": str(best.get("short_answer_candidate", "")),
            "confidence": best.get("confidence", 0.0),
            "evidence_sample_ids": [best.get("sample_id", "")] if best.get("sample_id") else [],
            "reasoning_summary": "Dry-run final answer from the highest ranked evidence result.",
        }
        raw = json.dumps(parsed, ensure_ascii=False)
    else:
        assert llm is not None
        raw = ask_llm(
            ...
            [
                {
                    "role": "system",
                    "content": "You are a careful final-answer generator for a focused RLM REPL.",
                },
                {"role": "user", "content": prompt},
            ],
    ...
    parsed.setdefault("long_answer", "")
    parsed.setdefault("answer_summary", parsed.get("short_answer", ""))
    parsed.setdefault("evidence_sample_ids", [])
    return parsed, raw, prompt
...
def run_focused_recursive_answer(
    llm: Llama | None,
    question: str,
    docs: list[Any],
    cfg: RlmConfig,
) -> Dict[str, Any]:
    """Run a fixed three-job REPL: understand, extract, generate."""
    env = make_environment(question, docs)
    state: Dict[str, Any] = {
        "environment": env,
        "sub_results": [],
    ...
    trace: List[Dict[str, Any]] = [
        {
            "node": "init_focused_repl_environment",
            "paper_principle": (
                "P is external. The focused REPL runs three jobs: "
                "understand_question, extract_answer_evidence, generate_final_answer."
            ),
            "metadata": env["metadata"],
        }
    ]
    ...
    print(
        "[focused-repl] InitREPL(P): "
        f"snippets={env['metadata']['snippet_count']} "
        f"chars={env['metadata']['total_snippet_chars']}",
        flush=True,
    )

    print("[focused-repl] Job 1/3 understand_question", flush=True)
    question_understanding, understanding_raw, understanding_prompt = understand_question(
        llm,
        question,
        env["metadata"],
        cfg,
    ...
    history.append(
        {
            "job": "understand_question",
            "stdout_metadata": question_understanding,
        }
    )
    trace.append(
        {
            ...
            "raw_response": understanding_raw,
            "parsed_response": question_understanding,
        }
    )

    print("[focused-repl] Job 2/3 extract_answer_evidence", flush=True)
    handles = ranked_handles_for_understanding(question, state, question_understanding)
    if not handles:
        handles = env["metadata"]["available_handles"]
    print(f"[focused-repl] Evidence handles={', '.join(handles)}", flush=True)
    snippets = {snippet["handle"]: snippet for snippet in env["P"]["snippets"]}
    for handle in handles:
        snippet = snippets.get(handle)
        if not snippet:
            continue
        print(
            f"[focused-repl] sub_RLM evidence handle={handle} "
            f"sample_id={snippet.get('sample_id', '')}",
            flush=True,
        )
        parsed, raw, prompt = sub_rlm_one(llm, question, snippet, cfg)
    ...
    history.append(
        {
            "job": "extract_answer_evidence",
            "stdout_metadata": {
                "selected_handles": handles,
                "sub_result_count": len(state["sub_results"]),
                "snippet_classifications": state.get("snippet_classifications", []),
            },
        }
    )

    print("[focused-repl] Job 3/3 generate_final_answer", flush=True)
    parsed_final, raw_final, generation_prompt = generate_final_answer(
        llm,
        question,
        state,
        question_understanding,
    ...
    state["root_prompt"] = generation_prompt
    state["raw_response"] = raw_final
    state["parsed_response"] = parsed_final
    state["final"] = parsed_final
    history.append(
        {
            "job": "generate_final_answer",
            "stdout_metadata": {
                "final_set": True,
                "short_answer": parsed_final.get("short_answer", ""),
    ...
    print(
        "[focused-repl] Complete: "
        f"sub_results={len(state['sub_results'])} "
        f"short_answer={str(parsed_final.get('short_answer', ''))[:120]}",
        flush=True,
    )
    return {
        "environment": env,
        "question_understanding": question_understanding,
        "question_understanding_prompt": understanding_prompt,
        "question_understanding_raw": understanding_raw,
        ...
        "repl_code_call_count": 0,
        "repl_step_count": 3,
        "question_understanding_call_count": 0 if cfg.dry_run else 1,
        "final_generation_call_count": 0 if cfg.dry_run else 1,
    }
...
def write_profile(df: pd.DataFrame, out_dir: Path) -> None:
    profile = {
        ...
        "method": (
            "Focused Algorithm-1 REPL-style RLM: InitREPL(P), then three "
            "answer-focused jobs: understand_question, extract_answer_evidence, "
            "and generate_final_answer. Stage outputs are appended to history "
            "and saved to CSV."
        ),
    }
...
def run(cfg: RlmConfig) -> None:
    print("[start] Focused three-job RLM REPL RAG evaluator", flush=True)
    ...
    for index, row in enumerate(df.to_dict("records"), start=1):
        ...
        result = run_focused_recursive_answer(llm, question, docs, cfg)
        gt = ground_truth(row)
        ...
        out_row = {
            "row_number": index,
            "experiment_name": "Focused RLM REPL + RAG",
            "row_explanation": (
                "One Natural Questions item evaluated with external P and a focused "
                "three-job REPL: understand question, extract answer evidence, "
                "generate final answer, then automatic metrics and LLM-as-judge."
            ),
            "sample_id": first_text(row.get("sample_id", "")),
            "question": question,
            "answer_summary": (
            ...
            "focused_repl_job_sequence": compact_json(
                [
                    "understand_question",
                    "extract_answer_evidence",
                    "generate_final_answer",
                ]
            ),
            "question_understanding_call_count": result.get("question_understanding_call_count", ""),
            "final_generation_call_count": result.get("final_generation_call_count", ""),
            "question_understanding_json": compact_json(result.get("question_understanding", {})),
            "question_understanding_prompt": result.get("question_understanding_prompt", ""),
            "question_understanding_raw": result.get("question_understanding_raw", ""),
            "selected_evidence_handles": compact_json(result.get("selected_evidence_handles", [])),
            "snippet_classifications_json": compact_json(result.get("snippet_classifications", [])),
            ...
            "repl_allowed_functions": compact_json(
                [
                    "understand_question(P.metadata)",
                    "extract_answer_evidence(P.snippets)",
                    "generate_final_answer(question_understanding, evidence)",
                ]
            ),
            "repl_algorithm_mapping": (
                "state=InitREPL(P); AddFunction(understand_question, "
                "extract_answer_evidence, generate_final_answer); "
                "hist+=Metadata(question understanding); hist+=Metadata(evidence); "
                "FINAL is set by focused final-answer generation."
            ),
            "trace_summary": (
                "init_focused_repl_environment -> understand_question -> "
                "extract_answer_evidence -> generate_final_answer -> "
                "LLM judge -> metrics"
            ),
        ...
        add_paper_columns(
            out_row,
            metric_prefix="rlm",
            method_mapping="Focused three-job REPL-style Recursive Language Model over external P snippets.",
            principles=(
                "Applies Recursive Language Models Algorithm 1 with external P, "
                "but narrows the REPL to three answer-focused jobs: understand "
                "the question, extract answer evidence, and generate the final answer."
            ),
            estimated_lm_calls=result.get("repl_code_call_count", 0)
            + result.get("question_understanding_call_count", 0)
            + len(result["sub_results"])
            + result.get("final_generation_call_count", 0)
    ...
    results_path = cfg.out_dir / "nq_rlm_repl_focused_assessment_traces.csv"
    ...
    summary.insert(1, "method", "FOCUSED_RLM_REPL_RAG")
    summary.insert(
        2,
        "summary_explanation",
        (
            "Mean metrics for focused Algorithm-1 REPL-style RLM: external P, "
            "question understanding, answer evidence extraction, final answer "
            "generation, metrics, and LLM-as-judge."
        ),
    )
    ...
    summary_path = cfg.out_dir / "nq_rlm_repl_focused_summary.csv"
