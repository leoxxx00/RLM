#!/usr/bin/env python3
"""Full-100 RLM REPL 8-step RAG evaluator for Natural Questions.
...
PROJECT_OUT_DIR = Path("/Users/htet/Desktop/Projects/X-RLM/nq_rlm_repl8_checkpoint_rag_outputs")
...
def run(cfg: RlmConfig) -> None:
    print("[start] Checkpointed RLM REPL 8-step RAG evaluator", flush=True)
    print(f"[start] Output directory: {cfg.out_dir}", flush=True)
    cfg.out_dir.mkdir(parents=True, exist_ok=True)
    results_path = cfg.out_dir / "nq_rlm_repl8_checkpoint_rag_assessment_traces.csv"
    summary_path = cfg.out_dir / "nq_rlm_repl8_checkpoint_rag_summary.csv"
    ...
    for index, row in enumerate(df.to_dict("records"), start=1):
        ...
        out_row = {
            "row_number": index,
            "experiment_name": "RLM REPL 8-step full100 + RAG",
            "row_explanation": (
