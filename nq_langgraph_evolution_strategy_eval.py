...
PROJECT_OUT_DIR = Path("/Users/htet/Desktop/Projects/X-RLM/nq_evolution_strategy_rag_outputs")
...
def tournament_select(scored: List[tuple[float, str]], tournament_size: int) -> tuple[float, str]:
    """Select one parent by tournament, without crossover."""
    size = max(1, min(tournament_size, len(scored)))
    contenders = random.sample(scored, size)
    contenders.sort(reverse=True, key=lambda item: item[0])
    return contenders[0]
...
def evolve_prompt(
    ...
    for generation in range(1, cfg.generations + 1):
        print(
            f"[evolution_strategy] Generation {generation}/{cfg.generations} "
            f"(elitism={cfg.keep_top}, tournament={cfg.tournament_size}, crossover=no)",
            flush=True,
        )
        scored = []
            ...
            history.append(
                {
                    ...
                    "score": score,
                    "selection": "evaluated",
                    "strategy": "mutation_only_tournament_elitism_no_crossover",
                    "prompt_preview": prompt.replace("\n", " ")[:240],
                }
            )
            print(f"[evolution_strategy] G{generation} P{index} score={score:.1f}", flush=True)
        scored.sort(reverse=True, key=lambda item: item[0])
        elites = [prompt for _, prompt in scored[: cfg.keep_top]]
        next_population = elites[:]
        for elite_rank, (score, prompt) in enumerate(scored[: cfg.keep_top], start=1):
            history.append(
                {
                    "generation": generation,
                    "candidate": f"elite_{elite_rank}",
                    "score": score,
                    "selection": "elitism",
                    "strategy": "keep_best_prompt_unchanged",
                    "prompt_preview": prompt.replace("\n", " ")[:240],
                }
            )
        while len(next_population) < cfg.population_size:
            parent_score, parent_prompt = tournament_select(scored, cfg.tournament_size)
            child = mutate_prompt(parent_prompt)
            next_population.append(child)
            history.append(
                {
                    "generation": generation,
                    "candidate": f"child_{len(next_population)}",
                    "parent_score": parent_score,
                    "selection": "tournament_mutation",
                    "strategy": "no_crossover",
                    "prompt_preview": child.replace("\n", " ")[:240],
                }
            )
...
def make_graph(vectorstore: Chroma, llm: Llama | None, cfg: EvalConfig):
    ...
    def prompt_node(state: RagState) -> RagState:
        ...
        if cfg.use_evolution_strategy and not cfg.dry_run:
        ...
        elif cfg.use_evolution_strategy:
            evolved_history = [{"dry_run": True, "note": "Evolution Strategy prompt evolution skipped in dry run."}]
        p_environment = {
...
def run(cfg: EvalConfig) -> None:
    ...
    for index, row in enumerate(df.to_dict("records"), start=1):
        ...
        out_row = {
            "row_number": index,
            "experiment_name": "Evolution Strategy + RAG",
            "row_explanation": (
                "One Natural Questions item evaluated with evolution strategy prompt search, "
            ...
            "use_evolution_strategy": cfg.use_evolution_strategy,
            "evolution_strategy_type": "mutation_only_tournament_selection_elitism_no_crossover",
            "tournament_size": cfg.tournament_size,
            "elitism_keep_top": cfg.keep_top,
            ...
            "trace_summary": (
                "retrieve -> evolution strategy prompt candidates -> prompt -> llm -> "
                "assess -> llm_judge"
            ),
        ...
        add_paper_columns(
            out_row,
            metric_prefix="rag",
            method_mapping="Evolution strategy prompt search, then RAG answer and judge.",
            principles=(
                "Applies compound inference scaling ideas from more-LM-calls paper; "
                "faithfulness metric fusion combines metrics; tournament selection and elitism explore prompt candidates without crossover."
            ),
            estimated_lm_calls=evolution_strategy_calls + 1 + (0 if cfg.skip_llm_judge else 1),
        )
        rows.append(out_row)
        print(
    ...
    summary.insert(1, "method", "EVOLUTION_STRATEGY_PROMPT_RAG")
    summary.insert(2, "summary_explanation", "Mean metrics for evolution strategy prompt RAG with P environment traces.")
...
def parse_args() -> EvalConfig:
    ...
    parser.add_argument("--results-csv", default="nq_evolution_strategy_rag_llama_assessment_traces.csv")
    parser.add_argument("--summary-csv", default="nq_evolution_strategy_rag_llama_summary.csv")
    parser.add_argument("--profile-csv", default="nq_evolution_strategy_dataset_profile.csv")
    ...
    parser.add_argument("--use-evolution-strategy", action="store_true")
    ...
    if args.tournament_size < 1:
        parser.error("--tournament-size must be at least 1")
    ...
    return EvalConfig(
        ...
        use_evolution_strategy=args.use_evolution_strategy,
