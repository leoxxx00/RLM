#!/usr/bin/env python3
"""Compare all available Natural Questions RAG/RLM model traces on first 50 rows.

This script does not rerun the LLM. It loads the existing per-question trace CSVs,
takes the first 50 rows from each available model, and computes comparison CSVs
and plots with automatic metrics, LLM-as-judge, runtime, and fused score.
"""
...
DEFAULT_OUT_DIR = PROJECT_DIR / "nq_clean_monitor_exports" / "all_models_first50"
...
def write_csvs(df: pd.DataFrame, out_dir: Path, max_rows: int) -> pd.DataFrame:
    ...
    long_path = out_dir / "all_models_first50_trace_rows_long.csv"
    ...
    summary_path = out_dir / "all_models_first50_summary.csv"
    ...
    chart_path = out_dir / "all_models_first50_chart_ready_long.csv"
...
def plot_quality(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("All available models, first 50 questions")
    ax.grid(axis="y", alpha=0.25)
    ax.legend(frameon=False, ncol=2)
    fig.tight_layout()
    path = out_dir / "all_models_first50_quality_metrics.png"
...
def plot_runtime(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Runtime, first 50 questions")
    ax.grid(axis="y", alpha=0.25)
    plt.xticks(rotation=20, ha="right")
    fig.tight_layout()
    path = out_dir / "all_models_first50_runtime.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def plot_fused(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Balanced fused ranking, first 50 questions")
    ax.grid(axis="y", alpha=0.25)
    plt.xticks(rotation=20, ha="right")
    fig.tight_layout()
    path = out_dir / "all_models_first50_balanced_fused_score.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def plot_short_long(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("Short vs long F1, first 50 questions")
    ax.grid(alpha=0.25)
    fig.tight_layout()
    path = out_dir / "all_models_first50_short_vs_long_f1.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
