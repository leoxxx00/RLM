#!/usr/bin/env python3
"""Compare SAMSum model runs and generate CSV/PNG outputs."""
...
OUT_ROOT = PROJECT_DIR / "samsum_outputs"
DEFAULT_OUT_DIR = PROJECT_DIR / "samsum_clean_monitor_exports" / "all_samsum_models"

TRACE_FILES = {
    "SAMSum Llama 3 8B + RAG": OUT_ROOT / "samsum_rag" / "samsum_rag_assessment_traces.csv",
    "SAMSum GA + RAG": OUT_ROOT / "samsum_ga_rag" / "samsum_ga_rag_assessment_traces.csv",
    "SAMSum ES + RAG": OUT_ROOT / "samsum_es_rag" / "samsum_es_rag_assessment_traces.csv",
    "SAMSum RLM REPL 4-step + RAG": OUT_ROOT / "samsum_rlm_repl4_rag" / "samsum_rlm_repl4_rag_assessment_traces.csv",
    "SAMSum RLM REPL 8-step + RAG": OUT_ROOT / "samsum_rlm_repl8_rag" / "samsum_rlm_repl8_rag_assessment_traces.csv",
    "SAMSum Adaptive-P short REPL + RAG": OUT_ROOT
    / "samsum_adaptive_p_short_repl_rag"
    / "samsum_adaptive_p_short_repl_rag_assessment_traces.csv",
}

MODEL_ORDER = list(TRACE_FILES)
COLORS = {
    "SAMSum Llama 3 8B + RAG": "#2ca02c",
    "SAMSum GA + RAG": "#ff7f0e",
    "SAMSum ES + RAG": "#d62728",
    "SAMSum RLM REPL 4-step + RAG": "#9467bd",
    "SAMSum RLM REPL 8-step + RAG": "#8c564b",
    "SAMSum Adaptive-P short REPL + RAG": "#bcbd22",
}
...
def read_all(max_rows: int) -> pd.DataFrame:
    ...
    if not frames:
        raise RuntimeError("No SAMSum trace files found.")
    return pd.concat(frames, ignore_index=True)


def summary_table(df: pd.DataFrame, max_rows: int, out_dir: Path) -> pd.DataFrame:
    out_dir.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_dir / "samsum_all_models_trace_rows_long.csv", index=False)
    ...
    summary.to_csv(out_dir / "samsum_all_models_summary.csv", index=False)
    return summary
...
def plot_quality(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("SAMSum model quality metrics")
    ax.grid(axis="y", alpha=0.25)
    ax.legend(frameon=False, ncol=2)
    fig.tight_layout()
    path = out_dir / "samsum_quality_metrics.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def plot_heatmap(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("SAMSum metric heatmap")
    fig.colorbar(image, ax=ax, fraction=0.025, pad=0.02)
    fig.tight_layout()
    path = out_dir / "samsum_metric_heatmap.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def plot_distributions(df: pd.DataFrame, out_dir: Path) -> None:
    for metric in ["quality_fused_score", "summary_f1", "summary_rouge1", "llm_judge_score_norm"]:
        ...
        ax.set_title(f"SAMSum distribution: {LABELS.get(metric, metric)}")
        ax.set_ylabel(LABELS.get(metric, metric))
        ax.grid(axis="y", alpha=0.25)
        plt.xticks(rotation=25, ha="right")
        fig.tight_layout()
        path = out_dir / f"samsum_{metric}_distribution.png"
        fig.savefig(path, dpi=180)
        plt.close(fig)
        print(f"[write] {path}")
...
def plot_scatter(summary: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_xlabel("Mean seconds per dialogue")
    ax.set_ylabel("Quality fused score")
    ax.set_title("SAMSum quality vs runtime")
    ax.grid(alpha=0.25)
    fig.tight_layout()
    path = out_dir / "samsum_fused_vs_runtime.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def plot_per_question_wins(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("SAMSum per-dialogue fused-score win count")
    ax.set_ylabel("Dialogue wins")
    ax.grid(axis="y", alpha=0.25)
    plt.xticks(rotation=25, ha="right")
    fig.tight_layout()
    path = out_dir / "samsum_per_dialogue_win_count.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    print(f"[write] {path}")
...
def main() -> None:
    ...
    bar_plot(summary, "quality_fused_score", "SAMSum quality fused score", args.out_dir / "samsum_quality_fused_score.png", (0, 1))
    bar_plot(summary, "retrieved_self_hit", "SAMSum retrieval self-hit rate", args.out_dir / "samsum_retrieval_self_hit.png", (0, 1))
    bar_plot(summary, "elapsed_seconds", "SAMSum average runtime per dialogue", args.out_dir / "samsum_runtime.png")
