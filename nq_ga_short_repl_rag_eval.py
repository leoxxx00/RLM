#!/usr/bin/env python3
"""GA + short REPL RAG evaluator for Natural Questions.
...
This hybrid first uses a small genetic algorithm to evolve a per-question
REPL policy prompt, then runs a short safe REPL loop over retrieved snippets.
All code/stdout/history is checkpointed to CSV after every question.
"""
...
PROJECT_OUT_DIR = Path("/Users/htet/Desktop/Projects/X-RLM/nq_ga_short_repl_rag_outputs")
...
def build_ga_base_policy(question: str, state: Dict[str, Any]) -> str:
    return (
        "Write a short policy for controlling the REPL on this question. "
        "The policy should choose efficient actions among find_snippets, "
        "batch_classify_snippets, sub_RLM, root_aggregate, and FINAL_VAR. "
        "Prefer 3-4 useful actions, avoid repeating the same action, and "
        "aggregate as soon as enough evidence is available.\n\n"
        f"Question:\n{question}\n\n"
        f"Metadata:\n{json.dumps(metadata_for_repl(state), ensure_ascii=False)}"
    )


def seed_ga_policies(question: str, state: Dict[str, Any]) -> List[str]:
    base = build_ga_base_policy(question, state)
    return [
        base + "\n\nPolicy: classify snippets, call sub_RLM only on likely evidence, then root_aggregate.",
        base + "\n\nPolicy: search precise keywords first, classify, extract only top snippets, then aggregate.",
        base + "\n\nPolicy: if first snippet matches the question, sub_RLM it immediately and aggregate early.",
        base + "\n\nPolicy: avoid repeated searches; after two failed searches call sub_RLM on all snippets.",
        base + "\n\nPolicy: prioritize long-answer evidence and exact short answer extraction.",
        base + "\n\nPolicy: use root_aggregate once at least one relevant sub_result has confidence above 0.8.",
    ]


def mutate_ga_policy(policy: str) -> str:
    mutations = [
        "Extra rule: do not repeat the same find_snippets keyword.",
        "Extra rule: call root_aggregate by the final REPL step.",
        "Extra rule: choose sub_RLM handles from the best classified snippets.",
        "Extra rule: use FINAL_VAR only if the JSON answer is already obvious.",
        "Extra rule: if all searches fail, use sub_RLM on every retrieved snippet.",
        "Extra rule: stop early when evidence is strong instead of using all steps.",
    ]
...
def crossover_ga_policy(parent_a: str, parent_b: str) -> str:
...
def score_ga_policy(llm: Llama, question: str, policy: str, cfg: RlmConfig) -> float:
    raw_score = ask_llm(
        llm,
        [
            {
                "role": "system",
                "content": (
                    "You are a strict evaluator for REPL control policies. "
                    "Score how well this policy will answer the question with "
                    "few useful REPL actions. Prefer precise evidence extraction, "
                    "early aggregation, and no repeated useless calls. Return only "
        ...
        temperature=0.1,
        max_tokens=cfg.ga_score_max_tokens,
    )
...
def evolve_ga_policy(
    llm: Llama | None,
    question: str,
    state: Dict[str, Any],
    cfg: RlmConfig,
) -> tuple[float, str, List[Dict[str, Any]]]:
    population = seed_ga_policies(question, state)[: cfg.population_size]
    while len(population) < cfg.population_size:
        population.append(mutate_ga_policy(random.choice(population)))

    if cfg.dry_run or llm is None:
        return 0.0, population[0], [{"dry_run": True, "note": "GA policy evolution skipped."}]

    history: List[Dict[str, Any]] = []
    for generation in range(1, cfg.generations + 1):
        print(f"[ga] Generation {generation}/{cfg.generations}", flush=True)
        scored = []
        for index, policy in enumerate(population, start=1):
            score = score_ga_policy(llm, question, policy, cfg)
            ...
            history.append(
                {
                    ...
                    "candidate": index,
                    "score": score,
                    "policy_preview": policy.replace("\n", " ")[:260],
                }
            )
            print(f"[ga] G{generation} P{index} score={score:.1f}", flush=True)
        scored.sort(reverse=True, key=lambda item: item[0])
        survivors = [policy for _, policy in scored[: cfg.keep_top]]
        next_population = survivors[:]
        while len(next_population) < cfg.population_size:
            parent_a, parent_b = random.sample(survivors, 2)
            next_population.append(mutate_ga_policy(crossover_ga_policy(parent_a, parent_b)))
        population = next_population

    final_scored = [(score_ga_policy(llm, question, policy, cfg), policy) for policy in population]
    final_scored.sort(reverse=True, key=lambda item: item[0])
    best_score, best_policy = final_scored[0]
    history.append({"final_best_score": best_score, "final_policy_preview": best_policy.replace("\n", " ")[:500]})
    return best_score, best_policy, history
...
def run_recursive_answer(
    ...
    ga_policy_score, ga_policy, ga_policy_history = evolve_ga_policy(llm, question, state, cfg)
    state["ga_policy"] = ga_policy
    state["ga_policy_score"] = ga_policy_score
    state["ga_policy_history"] = ga_policy_history
    history: List[Dict[str, Any]] = [
        {
            "type": "metadata",
    ...
    trace.append(
        {
            "node": "ga_policy_evolution",
            "ga_policy_score": ga_policy_score,
            "ga_policy": ga_policy,
            "ga_policy_history": ga_policy_history,
        }
    )
    print(
        f"[ga] Best short-REPL policy score={ga_policy_score:.1f}",
        flush=True,
    )
    print(
        "[repl] InitREPL(P): "
...
def write_summary(results: pd.DataFrame, summary_path: Path) -> None:
    ...
    summary.insert(1, "method", "RLM_REPL8_CHECKPOINT_RAG")
    summary.insert(
        2,
        "summary_explanation",
        (
            "Checkpointed 8-step Algorithm-1 REPL-style RLM. The trace CSV is "
            "saved after each question so interrupted runs can resume."
        ),
    )
...
def run(cfg: RlmConfig) -> None:
    print("[start] GA + short REPL RAG evaluator", flush=True)
    print(f"[start] Output directory: {cfg.out_dir}", flush=True)
    cfg.out_dir.mkdir(parents=True, exist_ok=True)
    results_path = cfg.out_dir / "nq_ga_short_repl_rag_assessment_traces.csv"
    summary_path = cfg.out_dir / "nq_ga_short_repl_rag_summary.csv"
    ...
    print(
        ...
        f"population={cfg.population_size} generations={cfg.generations} "
    ...
    for index, row in enumerate(df.to_dict("records"), start=1):
        ...
        out_row = {
            "row_number": index,
            "experiment_name": "GA + short REPL + RAG",
            "row_explanation": (
                "One Natural Questions item evaluated with a genetic search over "
                "short REPL control policies, external P snippets, recursive "
                "Algorithm-1 REPL actions, sub_RLM calls, root aggregation, "
                "automatic metrics, and LLM-as-judge."
            ),
            "sample_id": first_text(row.get("sample_id", "")),
            "question": question,
            "answer_summary": (
            ...
            "ga_policy_score": result.get("ga_policy_score", ""),
            "ga_policy": result.get("ga_policy", ""),
            "ga_policy_history_json": compact_json(result.get("ga_policy_history", [])),
            ...
            "trace_summary": (
                ...
                "GA evolves REPL policy -> LLM proposes REPL code/action -> "
        ...
        add_paper_columns(
            out_row,
            metric_prefix="rlm",
            method_mapping="GA-evolved short REPL policy over external P snippets.",
            principles=(
                "Uses genetic prompt/policy search to select a compact REPL "
                "control policy, then applies Recursive Language Models Algorithm "
                "1 with external P, safe tool/function execution, stdout metadata "
                "feedback, sub_RLM recursion, and a short REPL budget."
            ),
            estimated_lm_calls=(cfg.population_size * cfg.generations + cfg.population_size)
            + result.get("repl_code_call_count", 0)
...
def parse_args() -> RlmConfig:
    ...
    parser.add_argument("--ga-score-max-tokens", type=int, default=16)
    parser.add_argument(
        "--no-resume",
        action="store_true",
    ...
    return RlmConfig(
        ...
        ga_score_max_tokens=args.ga_score_max_tokens,
